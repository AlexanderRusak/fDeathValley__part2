...
const getModalValues = () => {
  const input_main_text = [
    getElementById("inputTitle").value,
    getElementById("inputTitle").value,
  ];
...
export const getColorFromSCSS = (element, color) => {
  return getComputedStyle(element).getPropertyValue(
    color === "black" ? "--color-font-black" : "--color-font-white"
  );
};

export const sortTodoList = (compare) => {
...
export const appendTodoElements = (completedStatus) => {
